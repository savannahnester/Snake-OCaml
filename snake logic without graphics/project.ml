...
let rec get_direction () =
  let input_direction = read_int() in
    match input_direction with
      1 -> DIRECTION_UP | 2 -> DIRECTION_DOWN | 3 -> DIRECTION_LEFT | 4 -> DIRECTION_RIGHT
    | _ -> get_direction();;
...
(*create a function called update snake, that can add the head and redirection it, or update the position somehow*)
